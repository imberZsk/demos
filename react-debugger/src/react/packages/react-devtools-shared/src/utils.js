...
export function castBool(v     )           {
  if (v === true || v === false) {
    return v;
  }
}

export function castBrowserTheme(v     )                {
...
export function getHideConsoleLogsInStrictMode()          {
  const raw = localStorageGetItem(
    LOCAL_STORAGE_HIDE_CONSOLE_LOGS_IN_STRICT_MODE,
  );
  return parseBool(raw) ?? false;
...
export function getShowInlineWarningsAndErrors()          {
  const raw = localStorageGetItem(
    LOCAL_STORAGE_SHOW_INLINE_WARNINGS_AND_ERRORS_KEY,
  );
  return parseBool(raw) ?? true;
...
export function getDefaultOpenInEditorURL()         {
  return typeof process.env.EDITOR_URL === 'string'
    ? process.env.EDITOR_URL
    : '';
}

export function getOpenInEditorURL()         {
  try {
