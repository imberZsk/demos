...
let lastBoundaryErrorComponentStackDev          = null;
...
function renderSuspenseBoundary(
  ...
  if (trackedPostpones !== null) {
    ...
    if (newBoundary.status === POSTPONED) {
      // This must exist now.
      const boundaryReplayNode                         =
        (trackedPostpones.workingMap.get(keyPath)     );
      ...
      if (typeof x === 'object' && x !== null) {
        ...
        if (
          ...
          const postponedSegment = injectPostponedHole(
            request,
            ((task     )            ), // We don't use ReplayTasks in prerenders.
...
export function stopFlowing(request         )       {
  request.destination = null;
}
...
export function flushResources(request         )       {
...
export function getResumableState(request         )                 {
  return request.resumableState;
}

export function getRenderState(request         )              {
...
export function getPostponedState(request         )                        {
  const trackedPostpones = request.trackedPostpones;
  if (
    trackedPostpones === null ||
